...
- Rust TrayIcon 左键切换 popover；右键显式弹出查看详情、设置、退出菜单，状态栏按钮不常驻绑定系统菜单。按下时锁定展开/收起意图，释放时执行，避免失焦事件与点击交错导致重新展开。标题直接由 Rust 的最新已完成投影生成，UI 未加载也可更新。
- popover 宽约 500px、高度 min(780px,当前屏幕工作区高度−边距)，使用托盘事件的物理坐标定位当前屏幕，按该屏缩放因子转换坐标，并限制在工作区内。验证外接屏、不同缩放、菜单栏自动隐藏和刘海区域。
