...
impl Store {
    ...
    pub fn rebuild(&mut self) -> Result<()> {
        ...
        for (provider, id) in &affected {
            tx.execute("DELETE FROM turn_evidence WHERE provider=?1 AND turn_key IN (SELECT CASE WHEN provider='codex' THEN 'turn:' || native_turn_id ELSE native_turn_id END FROM event_facts WHERE provider=?1 AND rollout_id=?2 AND native_turn_id IS NOT NULL)",params![provider,id])?;
            tx.execute("DELETE FROM turns WHERE provider=?1 AND record_source='parsed' AND native_turn_id IN (SELECT native_turn_id FROM event_facts WHERE provider=?1 AND rollout_id=?2 AND native_turn_id IS NOT NULL)",params![provider,id])?;
        }
        for t in turns {
